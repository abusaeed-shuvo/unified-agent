...
@pytest.mark.asyncio
async def test_tools_key_omitted_when_no_tools_passed():
    """The 'tools' key is absent from the request payload when tools=None."""
    captured: dict = {}

    def capturing_handler(request: httpx.Request) -> httpx.Response:
        # Parse the JSON body and stash it for inspection.
    ...
    assert "tools" not in captured["body"], (
        "'tools' key should be absent when no tools are provided"
    )
...
@pytest.mark.asyncio
async def test_tools_key_present_when_tools_provided():
...
@pytest.mark.asyncio
async def test_serialise_message_includes_tool_call_id():
    """tool_call_id is included only when set."""
    adapter = LMStudioAdapter(
        base_url="http://localhost:1234",
        model="test-model",
    )

    msg_without = Message(role="user", content="Hello")
    assert adapter._serialise_message(msg_without) == {
        "role": "user",
        "content": "Hello",
    }

    msg_with = Message(role="tool", content="Result", tool_call_id="call_42")
    assert adapter._serialise_message(msg_with) == {
        "role": "tool",
        "content": "Result",
